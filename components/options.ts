...
export default class OptionsComponent extends HTMLElement {
  ...
  async connectedCallback() {
    this.innerHTML = `
      ...
      <div class="card">
        <div class="vertical">
          <label for="list-favorite">Favorite List (\`Name|link|svg\` for each row)</label>
  ...
  async restoreOptions(options: Record<string, unknown>) {
    ...
    if (options.favList)
      this.favLiElement.value = options.favList.toString().trim();
    if (options.favOpen)
  ...
  listenOptions() {
    ...
    this.favLiElement.addEventListener("change", () => {
      storage.setMeta("local:preference", {
        favList: this.favLiElement.value,
      });
    });
    this.favOpElement.addEventListener("change", () => {
      storage.setMeta("local:preference", {
